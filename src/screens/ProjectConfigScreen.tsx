...
import { ArrowLeft, Bell, Bot, ChevronRight, FileText, MessageSquare, Settings, Shield, UserCheck, Zap } from 'lucide-react-native';
...
export function ProjectConfigScreen({ projectId, session, onBack, onOpenAgent, onOpenContext }: Props) {
  ...
  const [updating, setUpdating] = useState<'case' | 'reply' | 'type' | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [caseResponse, settingsResponse] = await Promise.all([getAutoCaseCreateStatus(session, projectId), getBotSettings(session, projectId)]);
